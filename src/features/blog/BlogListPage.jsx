...
import { Newblog } from "@/shared/ui/BlogCardGrid";
import { motion, AnimatePresence } from "framer-motion";
import { FiSearch, FiBookOpen, FiAlertCircle, FiX } from "react-icons/fi";
      ...
      <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div>
              <h1 className="text-3xl md:text-4xl font-bold text-white flex items-center gap-3">
                ...
                <span className="text-sm text-gray-500 bg-white/5 px-3 py-1 rounded-full border border-white/10 ml-2">
                  {filteredData?.length || 0} articles
                </span>
              </h1>
            ...
            {searchTerm && (
              <div className="text-sm text-gray-400 bg-white/5 px-4 py-2 rounded-full border border-white/10">
                Found {filteredData?.length || 0} results
              </div>
            )}
          </div>
        </motion.div>
        ...
        >
          <p className="text-gray-400 text-sm">
            Showing {currentItems?.length || 0} of {filteredData?.length || 0}{" "}
            articles
          </p>
        ...
        {error && (
          ...
          >
            ...
            <button
              onClick={fetchData}
              className="mt-4 px-6 py-3 bg-red-500/20 border border-red-400/50 rounded-xl text-red-400 hover:bg-red-500/30 transition-all duration-300"
            >
              Try Again
            </button>
          </motion.div>
            ...
            {currentItems?.length > 0 ? (
              ...
              >
                <div className="w-20 h-20 rounded-full bg-gray-700/30 flex items-center justify-center mb-4">
                  <FiBookOpen className="w-10 h-10 text-gray-500" />
                </div>
        ...
        {!loading && !error && currentItems?.length > 0 && (
          ...
          >
            {filteredData?.length || 0} total articles • Page {currentPage} of{" "}
            {totalPages || 1}
          </motion.div>
        )}
      </div>
